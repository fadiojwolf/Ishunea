            ...
            <div className='fixed inset-0 overflow-y-auto'>
                ...
                >
                    <Dialog.Panel className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto transform rounded-2xl bg-white p-6 text-left shadow-xsl transition-all flex flex-col gap-5">
                        <button 
                            type='button'
                            className='absolutet top-2 right-2 z-10 w-fit p-2 bg-primary-blue-100 rounded-full'
                            onClick={closeModal}
                        >
                            <Image 
                                src="/close.svg"
                        ...
                        <div className='flex-1 flex flex-col gap-3'>
                            <div className="relative w-full h-40 bg-patern bg-cover bg-center rounded-lg">
                        ...
                        <div className='flex-1 flex flex-col gap-2'>
                            <h2 className='font-semibold text-xl copitalize'>
                                {car.make} {car.model}
                            </h2>
                            <div className='mt-3 flex flex-wrap gap-4'>
                                {Object.entries(car).map(([key, volue]) => (
                                    <div className='flex justify-between gap-5 w-full text-right' key={key}>
                                        <h4 className='text-grey capitalize'>{key.split("_").join(" ")}</h4>
                                        <p className='text-black font-semibold'>{volue}</p>
                                    </div>
                                ))}
